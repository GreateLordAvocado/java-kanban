...
class InMemoryTaskManagerTest extends TaskManagerTest<InMemoryTaskManager> {
    ...
    public void epicCannotAddItselfAsASubtask() {
        Epic epic = new Epic(1, "ЭпикТест 1", "Описание эпикТеста 1", TaskStatus.NEW);
    ...
    void subtaskCannotBeItsOwnEpic() {
        Epic epic = new Epic(1, "ЭпикТест 2", "Описание эпикТеста 2", TaskStatus.NEW);
        taskManager.createEpic(epic);
        Subtask subtask = new Subtask(1, "Подзадача 1", "Описание подзадачи 1", TaskStatus.NEW, 1);
    ...
    public void checkTaskForImmutability() {
        ...
        assertEquals(task.getName(), newTask.getName(), "Название задачи не должно меняться!");
    ...
    public void checkEpicForImmutability() {
        Epic epic = new Epic(1, "Новый эпик", "Описание нового эпика", TaskStatus.NEW);
        taskManager.createEpic(epic);
        Epic newEpic = taskManager.getEpicById(1);
        assertEquals(epic.getId(), newEpic.getId(), "id эпика не должен меняться!");
        assertEquals(epic.getName(), newEpic.getName(), "Название эпика не должно меняться!");
    ...
    public void checkSubtaskForImmutability() {
        ...
        Subtask subtask = new Subtask(2, "Подзадача для эпика",
                "Описание подзадачи", TaskStatus.NEW, epicForSubtasks.getId());
        taskManager.createSubtask(subtask);
        ...
        assertEquals(subtask.getName(), newSubtask.getName(), "Название подзадачи не должна меняться!");
    ...
    public void getHistoryShouldReturnOldTaskAfterUpdate() {
        ...
        taskManager.updateTask(new Task(task.getId(), "Обновить задачу 1",
                "Описание обновленной задачи", TaskStatus.IN_PROGRESS));
    ...
    public void getHistoryShouldReturnOldEpicAfterUpdate() {
        ...
        taskManager.updateEpic(new Epic(epic.getId(), "Обновить эпик 1",
                "Описание обновленного эпика", TaskStatus.DONE));
    ...
    public void getHistoryShouldReturnOldSubtaskAfterUpdate() {
        ...
        taskManager.updateSubtask(new Subtask(subtask.getId(), "Обновить подзадачу",
                "Описание обновленной подзадачи", TaskStatus.IN_PROGRESS, epic2.getId()));
